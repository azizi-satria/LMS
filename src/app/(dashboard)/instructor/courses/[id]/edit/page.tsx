...
export default function EditCoursePage() {
  ...
  async function handleAddMaterial(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/materials", {
        ...
        body: JSON.stringify({
          ...newMaterial,
          order: (moduleObj?.materials.length || 0) + 1,
        }),
      });
      if (res.ok) {
  ...
  return (
    <div className="max-w-4xl">
      ...
      >
        {["info", "modules"].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab as "info" | "modules")}
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <div
                  ...
                  style={{
                    background: courseForm.isPublished
                      ? "linear-gradient(135deg, #7c3aed, #06b6d4)"
                      : "rgba(255,255,255,0.1)",
                  }}
                >
                  <div
                    className="w-4 h-4 rounded-full transition-all bg-white"
                    style={{ transform: courseForm.isPublished ? "translateX(20px)" : "translateX(0)" }}
                  />
                </div>
              </div>
              <label className="text-sm font-medium cursor-pointer" style={{ color: "#cbd5e1" }}>
                Publikasikan kursus
              </label>
            </div>
            <button
              type="submit"
              disabled={saving}
              className="gradient-btn py-3 px-6 rounded-xl font-semibold disabled:opacity-50"
            >
          ...
          {course.modules.map((module) => (
            <div key={module.id} style={cardStyle} className="overflow-hidden">
              <div
                ...
                style={{ background: "rgba(255,255,255,0.02)", borderBottom: "1px solid #1e1e2e" }}
              >
                ...
                {newMaterial?.moduleId === module.id ? (
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(e) => setNewMaterial({ ...newMaterial, type: e.target.value as "VIDEO" | "PDF" | "DOCUMENT" | "LINK" })}
                      style={{ ...inputStyle, background: "#111118" }}
                      className="dark-input"
                    >
                      {materialTypes.map((t) => (
                        <option key={t} value={t} style={{ background: "#111118" }}>{materialTypeLabels[t]}</option>
                      ))}
                    </select>
                    <input
                      type="url"
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="gradient-btn px-4 py-2 rounded-xl text-sm font-medium disabled:opacity-50"
                      >
                        {addingMaterial ? "Menambahkan..." : "Tambah"}
                      </button>
                      <button
                        type="button"
                  ...
                  <button
                    onClick={() =>
                      setNewMaterial({ moduleId: module.id, title: "", type: "VIDEO", contentUrl: "" })
                    }
                    className="w-full px-6 py-3 text-sm text-left flex items-center gap-2 transition-all"
                    style={{
                      color: "#a855f7",
                      borderTop: "1px solid rgba(255,255,255,0.04)",
                    }}
          ...
          <div style={cardStyle} className="p-6">
            ...
            <form onSubmit={handleAddModule} className="flex gap-3">
              ...
              <button
                ...
                className="gradient-btn px-4 py-2.5 rounded-xl text-sm font-medium disabled:opacity-50 whitespace-nowrap"
              >
